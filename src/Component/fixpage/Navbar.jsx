...
const Navbar = () => {
  ...
  return (
    <>
      <nav className="fixed top-0 left-0 w-full bg-green-800 shadow-md z-50">
        <div className="max-w-[95%] mx-auto py-4 flex justify-between items-center">
          ...
          {mobileMenu && (
            <div className="absolute top-full left-0 w-full bg-green-900 text-white flex flex-col py-5 px-6 md:hidden gap-4 shadow-lg">
              ...
              {user && <NavLink onClick={() => setMobileMenu(false)} to="/cart" className="hover:text-yellow-300 flex items-center gap-2 text-2xl"><FaShoppingCart /></NavLink>}
            </div>
          )}
        </div>
      </nav>
